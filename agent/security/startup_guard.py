...
configured — a public bind with no token would expose /api/chat and every
tool it can reach to the open network.

This is the bind-time half of the check. The request-time half is
agent/security/auth.py's bearer_auth_middleware, which validates that same
token on every /api/ request, so §2.1 (bearer token rotation) is now
meaningful: rotating the token revokes old holders immediately. §1.4 (auth
rate limiting) is still unbuilt.
"""
