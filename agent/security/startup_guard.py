...
configured, since there is currently no auth surface at all — a public bind
today would expose /api/chat and every tool it can reach to the open
network. This single guard is also why §1.4 (auth rate limiting) and §2.1
(bearer token rotation) are safely skippable: there's no non-loopback
listener for them to protect until this guard is satisfied.
"""
